...
def rtmp():
    ...
    try:
        subprocess.run(["rtmpdump", "-r",
                        ...
                        "-B", TIME_OUT, "-o", file_name+FILE_FLV])
...
def mpc():
    """KBS 방송을 녹음하는 함수"""
    file_name = FILE_PATH+__get_current_time('day') + FILE_KBS
    ...
    try:
        process = subprocess.Popen(
            "mplayer $("+curl_KBS+") -ao pcm:file="+file_name+FILE_MP3+" -vc dummy -vo null",
            shell=True)
...
def __get_pid():
    get_process = subprocess.check_output("ps | grep mplayer", shell=True, stderr=subprocess.STDOUT)
...
def __change_meta_data(file_path):
    ...
    :param: mp3파일 이름
    """
    file_path = '{}/{}'.format(file_path.split("/")[-2], file_path.split("/")[-1])
    try:
        meta = EasyID3(file_path)
    except mutagen.id3.ID3NoHeaderError:
        meta = mutagen.File(file_path, easy=True)
        meta.add_tags()
    meta['title'] = __get_date_from_file(file_path, "name")+"_kimSooHyun"
